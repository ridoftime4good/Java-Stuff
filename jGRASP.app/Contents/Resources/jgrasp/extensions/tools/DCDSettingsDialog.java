    ...
    public class DCDSettingsDialog {
       ...
       *  @return the newly created dialog. **/
       private JDialog createDialog(final Frame parent) {
         ...
         label = new JLabel(
               "Excluded Classes (comma-separated regular expressions)");
         contentPanel.add(label);
         insets.left = spacing;
         layout.setConstraints(label, constraints);
      
         excludedClassesField = new JTextField();
         contentPanel.add(excludedClassesField);   
         insets.top = 0;
         layout.setConstraints(excludedClassesField, constraints);
         
         label = new JLabel(
               "Excluded Methods (comma-separated regular expressions)");
         contentPanel.add(label);
         insets.top = spacing;
         layout.setConstraints(label, constraints);
      
         excludedMethodsField = new JTextField();
