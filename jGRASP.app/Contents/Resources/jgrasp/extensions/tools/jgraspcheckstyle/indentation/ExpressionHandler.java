    ...
    public abstract class ExpressionHandler
   {
     ...
     */
       public final IndentLevel getLevel()
      {
       ...
       public final IndentLevel getModsLevel()
      {
         if (modsLevel == null) {
            modsLevel = getModsLevelImpl();
         }
         return modsLevel;
      }
     ...
     */
       protected IndentLevel getLevelImpl()
      {
       ...
       protected IndentLevel getModsLevelImpl() {
         if (!isCSDMode() || !modsShift() || noSymIndent()) {
            return getLevel();
         }
         return new IndentLevel(getLevel(), 1);
      }
     ...
     */
       protected final void checkModifiers()
      {
             ...
             modifier = (DetailAST) modifier.getNextSibling())
         {
            ...
            */
            int shift = 0;
            if (isCSDMode() && modsShift()) {
               shift = 1;
            }
            if (startsLine(modifier)
                && !getModsLevel().accept(expandedTabsColumnNo(modifier)))
            {
               logError(modifier, "modifier",
                    expandedTabsColumnNo(modifier), getModsLevel());
            }
         }
      }
   
      /** Determines if modifiers may be shifted by 1 space to the
       *  right. A CSD with symbols will do this, one without will
       *  not.
       *
       *  @return true if modifiers may be shifted, false otherwise. **/
       protected boolean modsShift() {
         return false;
      }
     ...
     * @return value of basicOffset property of <code>IndentationCheck</code>
     */
       protected final boolean isCSDMode()
      {
         return getIndentCheck().isCSDMode();
      }
       ...
       *  by an extra space, false otherwise. **/
       protected final boolean noSymIndent() {
         return getIndentCheck().noSymIndent();
      }
     ...
     */
       protected final void checkRParen(DetailAST aLparen, DetailAST aRparen)
      {
        ...
        // or not first on the line ...
         ...
         int shift = 0;
         if (isCSDMode()) {
            shift = getBasicOffset();
         }
         if (getLevel().accept(rparenLevel - shift) || !startsLine(aRparen)) {
            return;
         }
